...
require.cache[require.resolve(path.join(servicesDir, 'calendar.js'))] = {
    ...
    exports: {
        getAvailableSlots: async () => [
            { dayLabel: 'Monday, April 7', date: '2026-04-07', times: ['10:00 AM', '2:00 PM'] },
        ],
...
describe('Server', () => {
    ...
    before(async () => {
        ...
        app.get('/api/health', (req, res) => res.json({ status: 'ok', service: 'cushlabs-ai-voice-agent' }));
    ...
    describe('Security headers', () => {
        it('returns all security headers', async () => {
            ...
            assert.ok(res.headers['content-security-policy'].includes('cdn.jsdelivr.net'));
            assert.ok(res.headers['content-security-policy'].includes('fonts.googleapis.com'));
            assert.ok(res.headers['content-security-policy'].includes('vitals.cushlabs.ai'));
        });
    });
    ...
    describe('GET /api/config', () => {
        ...
        it('falls back to cushlabs for unknown service', async () => {
            const res = await request(server, 'GET', '/api/config?service=nonexistent');
            assert.equal(res.body.assistantId, 'test-cushlabs-id');
        });
    });
    ...
    describe('POST /api/contact', () => {
        ...
        it('rejects missing fields', async () => {
            const res = await request(server, 'POST', '/api/contact', { name: 'Test' });
        ...
        it('rejects invalid email', async () => {
            const res = await request(server, 'POST', '/api/contact', {
                name: 'Test', email: 'bad', message: 'Hello',
            });
            assert.equal(res.status, 400);
        ...
        it('handles check_availability', async () => {
            const res = await request(server, 'POST', '/api/webhook', {
                message: {
                    type: 'function-call',
                    call: { id: 'call-1' },
                    functionCall: { id: 'fc-1', name: 'check_availability', parameters: {} },
                },
            }, AUTH);
            assert.equal(res.status, 200);
        ...
        it('handles book_appointment — missing params', async () => {
            const res = await request(server, 'POST', '/api/webhook', {
                message: {
                    type: 'function-call',
                    call: { id: 'call-2' },
                    functionCall: { id: 'fc-2', name: 'book_appointment', parameters: { caller_name: 'Test' } },
                },
            }, AUTH);
            assert.equal(res.status, 200);
        ...
        it('handles book_appointment — full params', async () => {
            const res = await request(server, 'POST', '/api/webhook', {
                message: {
                    ...
                    call: { id: 'call-3' },
                    functionCall: {
                        id: 'fc-3', name: 'book_appointment',
                        parameters: { caller_name: 'Jane', caller_email: 'j@test.com', date_time: '2026-04-07T10:00:00-06:00' },
                    },
                },
            }, AUTH);
            assert.equal(res.status, 200);
        ...
        it('handles qualify_lead', async () => {
            const res = await request(server, 'POST', '/api/webhook', {
                message: {
                    ...
                    call: { id: 'call-4' },
                    functionCall: {
                        id: 'fc-4', name: 'qualify_lead',
                        parameters: { user_name: 'John', business_type: 'Plumbing', ai_interest: 'Scheduling' },
                    },
                },
            }, AUTH);
            assert.equal(res.status, 200);
        ...
        it('handles lookup_property — found', async () => {
            ...
            const res = await request(server, 'POST', '/api/webhook', {
                message: {
                    type: 'function-call',
                    call: { id: 'call-5' },
                    functionCall: { id: 'fc-5', name: 'lookup_property', parameters: { propertyId: mockMLS[0].id } },
                },
            }, AUTH);
            assert.equal(res.status, 200);
        ...
        it('handles lookup_property — not found', async () => {
            const res = await request(server, 'POST', '/api/webhook', {
                message: {
                    type: 'function-call',
                    call: { id: 'call-6' },
                    functionCall: { id: 'fc-6', name: 'lookup_property', parameters: { propertyId: 'FAKE' } },
                },
            }, AUTH);
            assert.equal(res.status, 200);
        ...
        it('handles unknown function call', async () => {
            const res = await request(server, 'POST', '/api/webhook', {
                message: {
                    type: 'function-call',
                    call: { id: 'call-8' },
                    functionCall: { id: 'fc-8', name: 'future_function', parameters: {} },
                },
            }, AUTH);
            assert.equal(res.status, 200);
